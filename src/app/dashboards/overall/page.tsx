...
export default function OverallDashboardPage() {
  ...
  return (
    <DashboardShell
      ...
      subtitle="Combined totals from all four sheets across recorded days"
      ...
      {!hasData ? (
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <KpiCard label="Total Production" value={s1?.totalProduction ?? 0} sub="Sheet 1 (all days)" />
            <KpiCard label="Total Wastage" value={s1?.totalWastage ?? 0} sub="Sheet 1 (all days)" />
            <KpiCard label="Wastage %" value={s1?.wastagePercentTotal ?? 0} suffix="%" />
            <KpiCard label="Efficiency A / B" value={`${s1?.efficiencyA ?? 0} / ${s1?.efficiencyB ?? 0}`} sub="Daily average" />
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <h2 className="mb-3 font-bold text-slate-900">Run MIS (cumulative)</h2>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div>
              <h2 className="mb-2 text-sm font-semibold text-slate-700">Shift A vs Shift B (Production, all days)</h2>
