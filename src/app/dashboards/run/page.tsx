...
export default function RunDashboardPage() {
  ...
  const totals = cumulative?.sheet2Totals;
  ...
  return (
    <DashboardShell
      title="Run Dashboard"
      subtitle="Sheet 2 - Run MIS (cumulative totals)"
      current="/dashboards/run"
        ...
        <>
          {totals ? (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <KpiCard label="Total Actual Run" value={totals.totalRun} sub="All materials, all days" />
              <KpiCard label="Total Planned Run" value={totals.totalRunPlanned} sub="All materials, all days" />
          ...
          {sheet2.map((row) => (
            <div key={row.material} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <KpiCard label="Shift A" value={row.shiftA} sub="Cumulative" />
                <KpiCard label="Shift B" value={row.shiftB} sub="Cumulative" />
          ...
          <div>
            <h2 className="mb-2 text-sm font-semibold text-slate-700">Daily Trend (all materials)</h2>
            <TrendChart
              data={trend}
              lines={[
